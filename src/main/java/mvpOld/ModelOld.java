...
import filesOld.LoaderOfDataOld;
import optimalizaciaOld.*;
...
public class ModelOld {
    ...
    private LinkedHashMap<DvojicaOld<Integer, Integer>, UsekOld> useky;
    private LinkedHashMap<DvojicaOld<Integer, Integer>, Integer> DT;
    private LinkedHashMap<DvojicaOld<Integer, Integer>, Integer> T;
    private LinkedHashMap<Integer, SpojOld> spoje;
    private LinkedHashMap<Integer, ZastavkaOld> zastavky;
    private LinkedHashMap<Integer, LinkaOld> linky;
    ...
    {
        LoaderOfDataOld loader = new LoaderOfDataOld();
        try
        {
        ...
        for (SpojOld spoj_i : this.spoje.values())
        {
            ...
            for (SpojOld spoj_j : this.spoje.values())
            {
                ...
                if (casPrichodu_i + casPrejazdu + REZERVA_1 <= casOdchodu_j)
                {
                    ...
                    this.DT.put(new DvojicaOld<>(spoj_i.getID(), spoj_j.getID()), casJazdy);
                    this.T.put(new DvojicaOld<>(spoj_i.getID(), spoj_j.getID()), casMedziSpojmi);
                ...
                int dist1 = this.useky.get(new DvojicaOld<>(miestoPrichodu_i, DEPO)).getCasPrejazdu();
                int dist2 = this.useky.get(new DvojicaOld<>(DEPO, miestoOdchodu_j)).getCasPrejazdu();
    ...
    public LinkedHashMap<Integer, String[][]> vypisVsetkyLinky()
    {
        ...
        for (LinkaOld linka_i: this.linky.values())
        {
    ...
    public String vykonajMinimalizaciuAutobusov(ArrayList<String[]> pTurnusyUdaje, ArrayList<String[][]> pSpojeUdaje)
    {
        try
        {
            MinimalizaciaAutobusovOld minBusov = new MinimalizaciaAutobusovOld(this.spoje);
            int pocetBusov = minBusov.getPocetAutobusov();
            ArrayList<TurnusOld> turnusy = minBusov.vytvorTurnusy(this.spoje);
                                                         ...
                                                         ArrayList<String[][]> pSpojeUdaje)
    {
        try
        {
            MinimalizaciaPrejazdovOld minBusov = new MinimalizaciaPrejazdovOld(pPocetBusov, this.spoje, this.useky);
            ...
            ArrayList<TurnusOld> turnusy = minBusov.getTurnusy();
            for (TurnusOld turnus: turnusy)
            {
                pTurnusyUdaje.add(turnus.getPrvaZmena().vypis(this.useky));
                pSpojeUdaje.add(turnus.getPrvaZmena().vypisSpoje(this.useky));
                                              ...
                                              ArrayList<String[]> pTurnusyUdaje, ArrayList<String[][]> pSpojeUdaje)
    {
        try
        {
            MinimalizaciaVodicovOld minVodicov = new MinimalizaciaVodicovOld(pPocetBusov, this.spoje, this.useky, this.DT, this.T);

            boolean bezChyby = false;
            int count = 2;
            while(!bezChyby)
            {
                System.out.println("----------------------------" + count + ". Iterácia ------------------------------------------");
                bezChyby = minVodicov.vytvorSkontrolujTurnusy(this.spoje, this.useky, this.DT, this.T);
                count++;
            }
            minVodicov.vypisPremenne();
            int pocetVodicov = minVodicov.getPocetVodicov();
            ArrayList<TurnusOld> turnusy = minVodicov.getTurnusy();
            for (TurnusOld turnus: turnusy)
            {
                pTurnusyUdaje.add(turnus.getPrvaZmena().vypis(this.useky));
                pSpojeUdaje.add(turnus.getPrvaZmena().vypisSpoje(this.useky));
                                                  ...
                                                  ArrayList<String[]> pTurnusyUdaje, ArrayList<String[][]> pSpojeUdaje)
    {
        try
        {
            MaximalizaciaObsadenostiOld maxObs = new MaximalizaciaObsadenostiOld(pPocetBusov, pPocetVodicov,this.linky, this.spoje, this.useky, this.DT, this.T);
            ...
            {
                System.out.println("----------------------------" + count + ". Iterácia ------------------------------------------");
                bezChyby = maxObs.vytvorSkontrolujTurnusy(this.spoje, this.useky, this.DT, this.T);
                count++;
            }
            maxObs.vypisPremenne();
            ArrayList<TurnusOld> turnusy = maxObs.getTurnusy();
            for (TurnusOld turnus: turnusy)
            {
                pTurnusyUdaje.add(turnus.getPrvaZmena().vypis(this.useky));
                pSpojeUdaje.add(turnus.getPrvaZmena().vypisSpoje(this.useky));
                                                        ...
                                                        ArrayList<String[]> pTurnusyUdaje, ArrayList<String[][]> pSpojeUdaje)
    {
        try
        {
            MaximalizaciaObsluzenychSpojovOld maxObs = new MaximalizaciaObsluzenychSpojovOld(y, pPocetBusov, pPocetVodicov,
            ...
            maxObs.vypisPremenne();
            ArrayList<TurnusOld> turnusy = maxObs.getTurnusy();
            System.out.println(maxObs.pocetObsSpojov());
            for (TurnusOld turnus: turnusy)
            {
                pTurnusyUdaje.add(turnus.getPrvaZmena().vypis(this.useky));
                pSpojeUdaje.add(turnus.getPrvaZmena().vypisSpoje(this.useky));
